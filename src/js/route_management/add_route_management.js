    ...
    if (latitudePointInput.value != "" && longitudePointInput.value != "") {
        if (pinType.value == "0" || pinType.value == "1") {
            ...
            if (markerPoint >= 2) {
                ...
                pinOption[2].disabled = false;
                pinOption[3].disabled = false;
            }

        } else if (pinType.value == "2" || pinType.value == "3") {
...
async function verifyInputForm() {
    ...
    if (onCheck == false) {
        ...
        dataPack.append("data", `{"nameStart": "${nameStartServiceInput.value.trim()}", "nameEnd": "${nameEndServiceInput.value.trim()}"}`);
        ...
        if (await !onCheckValidate) {
            ...
            if (useRouteColor_2 == true) {
                ...
                nameInput.setAttribute("class", "form-control text-center using-custom-font custom-input-disabled");
...
async function saveData() {
    ...
    const serviceCountRoundVerify = document.querySelector("#verify_service_count_round");
    const serviceTimeAverageVerify = document.querySelector("#verify_service_time_average");
    const routeForwardVerify = document.querySelector("#verify_data_route_forward");
    const routeBackwardVerify = document.querySelector("#verify_data_route_backward");
    const serviceRouteStatusVerify = document.querySelector("#verify_service_route_status");

    nameRouteServiceVerify.classList.remove(classToRemove[0]);
    routeKeywordDetailVerify.classList.remove(classToRemove[0]);
    titleRouteColorVerify.classList.remove(classToRemove[0]);
    tabRouteColorVerify.classList.remove(classToRemove[0]);
    nameColorRoute_1_Verify.classList.remove(classToRemove[0]);
    previewColorRoute_1_Verify.classList.remove(classToRemove[0]);
    pricePassengerLowerVerify.classList.remove(classToRemove[0]);
    pricePassengerHigherVerify.classList.remove(classToRemove[0]);
    priceSuppliesLowerVerify.classList.remove(classToRemove[0]);
    priceSuppliesHigherVerify.classList.remove(classToRemove[0]);
    serviceStartTimeVerify.classList.remove(classToRemove[0]);
    serviceEndTimeVerify.classList.remove(classToRemove[0]);
    serviceTimeRoundVerify.classList.remove(classToRemove[0]);
    serviceCountRoundVerify.classList.remove(classToRemove[0]);
    serviceTimeAverageVerify.classList.remove(classToRemove[0]);
    routeForwardVerify.classList.remove(classToRemove[0]);
    routeBackwardVerify.classList.remove(classToRemove[0]);
    serviceRouteStatusVerify.classList.remove(classToRemove[0]);

    if (useRouteColor_2 == true) {
        document.querySelector("#verify_name_color_route_2").classList.remove(classToRemove[0]);
        document.querySelector("#verify_preview_color_route_2").classList.remove(classToRemove[0]);
    }
    ...
    if (await statusSendData == "re-Send" || await statusSendData == "TypeError: Failed to fetch") {
        nameRouteServiceVerify.classList.add(classToRemove[0]);
        routeKeywordDetailVerify.classList.add(classToRemove[0]);
        titleRouteColorVerify.classList.add(classToRemove[0]);
        tabRouteColorVerify.classList.add(classToRemove[0]);
        nameColorRoute_1_Verify.classList.add(classToRemove[0]);
        previewColorRoute_1_Verify.classList.add(classToRemove[0]);
        pricePassengerLowerVerify.classList.add(classToRemove[0]);
        pricePassengerHigherVerify.classList.add(classToRemove[0]);
        priceSuppliesLowerVerify.classList.add(classToRemove[0]);
        priceSuppliesHigherVerify.classList.add(classToRemove[0]);
        serviceStartTimeVerify.classList.add(classToRemove[0]);
        serviceEndTimeVerify.classList.add(classToRemove[0]);
        serviceTimeRoundVerify.classList.add(classToRemove[0]);
        serviceCountRoundVerify.classList.add(classToRemove[0]);
        serviceTimeAverageVerify.classList.add(classToRemove[0]);
        routeForwardVerify.classList.add(classToRemove[0]);
        routeBackwardVerify.classList.add(classToRemove[0]);
        serviceRouteStatusVerify.classList.add(classToRemove[0]);

        if (useRouteColor_2 == true) {
            document.querySelector("#verify_name_color_route_2").classList.add(classToRemove[0]);
            document.querySelector("#verify_preview_color_route_2").classList.add(classToRemove[0]);
        }
...
async function sendData() {
    ...
    dataPack.append("nameStartService", nameStartServiceInput.value.trim());
    dataPack.append("nameEndService", nameEndServiceInput.value.trim());
    dataPack.append("detailRoute", routeKeywordDetailInput.value.trim());
    dataPack.append("nameColor_1", nameColor_1_Input.value.trim());
    dataPack.append("hexColor_1", hexColor_1_Input.value.trim());
    dataPack.append("pricePassengerLower", pricePassengerLowerInput.value.trim());
    dataPack.append("pricePassengerHigher", pricePassengerHigherInput.value.trim());
    dataPack.append("priceSuppliesLower", priceSuppliesLowerInput.value.trim());
    dataPack.append("priceSuppliesHigher", priceSuppliesHigherInput.value.trim());
    dataPack.append("serviceStartTime", serviceStartTimeInput.value.trim());
    dataPack.append("serviceEndTime", serviceEndTimeInput.value.trim());
    dataPack.append("serviceTimeRound", serviceTimeRoundInput.value.trim());
    dataPack.append("serviceCountRound", serviceCountRoundInput.value.trim());
    dataPack.append("serviceTimeAverage", serviceTimeAverageInput.value.trim());
    dataPack.append("dataRouteLocation", JSON.stringify(mapRouteLocation));

    if (routeDataMaster[0].length > 0) {
        dataPack.append("dataRouteForward", JSON.stringify(routeDataMaster[0][0]));
    }

    if (routeDataMaster[1].length > 0) {
        dataPack.append("dataRouteBackward", JSON.stringify(routeDataMaster[1][0]));
    }

    if (useRouteColor_2 == true) {
        dataPack.append("nameColor_2", document.querySelector("input[name='name_color_2']").value.trim());
        dataPack.append("hexColor_2", document.querySelector("input[name='hex_color_picker_2']").value.trim());
    } else {
        dataPack.append("nameColor_2", "");
        dataPack.append("hexColor_2", "");
    }

    const returnValue = await new Promise((resolve, reject) => {
        fetch(`${urlAPIServer}/route_management`, {
                method: 'POST',
